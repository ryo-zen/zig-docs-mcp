...
`std.Io.IoUring` is a high-performance, asynchronous I/O backend for Linux based on the `io_uring` kernel interface. Unlike thread-pool based approaches, `io_uring` allows applications to submit I/O requests to the kernel and retrieve completions asynchronously without the overhead of context switches or blocking syscalls.
...
- `std.os.linux.io_uring` - Raw bindings to the io_uring subsystem.
