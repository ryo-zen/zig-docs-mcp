...
```
Shell$ zig test test_structs.zig
...
```
Shell$ zig test struct_default_field_values.zig
...
```
Shell$ zig build-exe bad_default_value.zig
$ ./bad_default_value
thread 3446528 panic: reached unreachable code
/home/ci/zig-bootstrap/out/host/lib/zig/std/debug.zig:419:14: 0x1031679 in assert (std.zig)
    if (!ok) unreachable; // assertion failure
       ^
/home/ci/zig-bootstrap/zig/doc/langref/bad_default_value.zig:8:15: 0x11ab51a in categorize (bad_default_value.zig)
  assert(t.maximum >= t.minimum);
        ^
/home/ci/zig-bootstrap/zig/doc/langref/bad_default_value.zig:19:42: 0x11aa59e in main (bad_default_value.zig)
    const category = threshold.categorize(0.90);
                                   ^
/home/ci/zig-bootstrap/out/host/lib/zig/std/start.zig:679:59: 0x11aab5d in callMain (std.zig)
...
```
Shell$ zig test test_packed_structs.zig
...
```
Shell$ zig test test_missized_packed_struct.zig
...
```
Shell$ zig test test_pointer_to_non-byte_aligned_field.zig
...
```
Shell$ zig test test_misaligned_pointer.zig
/home/ci/zig-bootstrap/zig/doc/langref/test_misaligned_pointer.zig:17:20: error: expected type '*const u3', found '*align(1:3:1) u3'
    try expect(bar(&bit_field.b) == 2);
             ^~~~~~~~~~~~
/home/ci/zig-bootstrap/zig/doc/langref/test_misaligned_pointer.zig:17:20: note: pointer host size '1' cannot cast into pointer host size '0'
/home/ci/zig-bootstrap/zig/doc/langref/test_misaligned_pointer.zig:17:20: note: pointer bit offset '3' cannot cast into pointer bit offset '0'
...
```
Shell$ zig test test_packed_struct_field_address.zig
...
```
Shell$ zig test test_bitOffsetOf_offsetOf.zig
...
```
Shell$ zig test test_overaligned_packed_struct.zig
...
```
Shell$ zig test test_aligned_struct_fields.zig
...
```
Shell$ zig test test_packed_struct_equality.zig
...
```
Shell$ zig build-exe struct_name.zig
$ ./struct_name
variable: struct_name.main.Foo
anonymous: struct_name.main__struct_30256
...
```
Shell$ zig test test_struct_result.zig
...
```
Shell$ zig test test_anonymous_struct.zig
...
and are referred to as "tuples". An empty tuple looks like `.{}` and can be seen in one of the [Hello World examples](#Hello-World).
...
```
Shell$ zig test test_tuples.zig
...
```
Shell$ zig build-exe destructuring_return_value.zig
