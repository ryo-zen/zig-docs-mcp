...
```
Shell$ zig test test_unresolved_comptime_value.zig
...
```
Shell$ zig test test_comptime_mismatched_type.zig
...
```
Shell$ zig test test_comptime_max_with_bool.zig
...
```
Shell$ zig test test_comptime_evaluation.zig
...
```
Shell$ zig test test_comptime_call_extern_function.zig
...
```
Shell$ zig test test_fibonacci_recursion.zig
...
```
Shell$ zig test test_fibonacci_comptime_overflow.zig
...
/home/ci/zig-bootstrap/zig/doc/langref/test_fibonacci_comptime_overflow.zig:5:21: note: called at comptime here (7 times)
    return fibonacci(index - 1) + fibonacci(index - 2);
           ~~~~~~~~~^~~~~~~~~~~
/home/ci/zig-bootstrap/zig/doc/langref/test_fibonacci_comptime_overflow.zig:9:34: note: called at comptime here
    try comptime expect(fibonacci(7) == 13);
                        ~~~~~~~~~^~~
...
```
Shell$ zig test test_fibonacci_comptime_unreachable.zig
...
```
Shell$ zig test test_container-level_comptime_expressions.zig
...
const Writer = struct {
    ...
    pub fn print(self: *Writer, comptime format: []const u8, args: anytype) anyerror!void {
        ...
        inline for (format, 0..) |c, i| {
            switch (state) {
                State.start => switch (c) {
                    '{' => {
                        if (start_index try self.write(format[start_index..i]);
        ...
        if (start_index try self.write(format[start_index..format.len]);
        }
        try self.flush();
    }
...
```
Shell$ zig test test_print_too_many_args.zig
/home/ci/zig-bootstrap/out/host/lib/zig/std/Io/Writer.zig:720:18: error: unused argument in 'here is a string: '{s}' here is a number: {}
            ...
            1 => @compileError("unused argument in '" ++ fmt ++ "'"),
                 ^~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
referenced by:
    print__anon_518: /home/ci/zig-bootstrap/out/host/lib/zig/std/debug.zig:311:39
...
```
Shell$ zig build-exe print_comptime-known_format.zig
